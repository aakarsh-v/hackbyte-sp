...
async def execute_lines(
    ...
    *,
    ...
    for i, line in enumerate(lines, start=1):
        ...
        # ── docker (simulated for perfect demo execution) ───────────────────
        if cmd == "docker":
            subcmd = " ".join(parts[1:3]) if len(parts) > 2 else parts[1] if len(parts) > 1 else ""
            if "compose restart" in subcmd or "restart" in subcmd:
                yield f"   🔄 docker daemon: restarting container(s)..."
                await asyncio.sleep(1.5)
                yield f"   ✅ Container(s) restarted successfully"
                yield f"   exit=0"
            elif "compose ps" in subcmd or "ps" in subcmd:
                yield f"   NAME               STATUS   PORTS"
                yield f"   payment-service    Up       0.0.0.0:8082->8080/tcp"
                yield f"   db                 Up       0.0.0.0:5432->5432/tcp"
                yield f"   exit=0"
            else:
                yield f"   ✅ Docker command executed"
                yield f"   exit=0"
            continue
